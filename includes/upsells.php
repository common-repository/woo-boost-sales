                ...
                <div class="wbs-content-inner">
                    ...
                    <div class="wbs-breadcrum">
                        <p class="wbs-notify_added wbs-title_style1">
                            <span class="wbs-icon-added"></span> <?php /* translators: %s: item count */
                            printf( _n( '<span class="wbs-notify_added-quantity">%s</span>  new item has been added to your cart', '<span class="wbs-notify_added-quantity">%s</span>  new items have been added to your cart', wp_kses_post( $this->quantity ), 'woo-boost-sales' ), wp_kses_post( $this->quantity ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                        </p>
                        <div class="wbs-header-right">
                            ...
                            <a href="<?php echo esc_url( $checkout_url ) ?>"
                               class="wbs-button-check">
								<?php
								echo wp_kses_post( apply_filters( 'woocommerce_boost_sales_upsells_checkout_text', esc_html__( 'Checkout', 'woo-boost-sales' ) ) );
								?>
                            </a>
                        </div>
                        <div class="wbs-product">
                            ...
                            <div class="wbs-p-title">
								...
								if ( $main_product_url ) {
									?>
                                    <a href="<?php echo esc_url( $main_product_url ) ?>" class="wbs-p-url"
                                       target="_self"><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( $product_title );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a>
									<?php
								} else {
									echo VI_WOO_BOOSTSALES_Data::wp_kses_post( $product_title );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								}
								?>
                            </div>
                                ...
                                <div class="wbs-price-total">
                                    <div class="wbs-total-price"><?php esc_html_e( 'Total', 'woo-boost-sales' ) ?>
                                        <span class="wbs-money"
                                              style="float: none;"><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( $total_product );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
					...
					if ( $upsells_count > 0 ) {
                        ...
                        <div class="wbs-bottom">
							...
							if ( $message_bought ) {
								?>
                                <h3 class="upsell-title"><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( str_replace( '{name_product}', $product_title, wp_strip_all_tags( $message_bought ) ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h3>
                                <hr/>
								<?php
							}
							/*upsells list here*/
