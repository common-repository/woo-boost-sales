...
if ( ! function_exists( 'woocommerce_boost_sales_prefix' ) ) {
	function woocommerce_boost_sales_prefix() {
		$prefix = get_option( '_woocommerce_boost_sales_prefix', date( "Ymd" ) );// phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date

		return $prefix . '_products_' . date( "Ymd" );// phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
	}
}
...
if ( ! function_exists( 'wbs_get_template' ) ) {
	...
	function wbs_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
		...
		if ( ! file_exists( $located ) ) {
			...
			wc_doing_it_wrong( __FUNCTION__, sprintf( __( '%s does not exist.', 'woo-boost-sales' ), '<code>' . $located . '</code>' ), '2.1' );
	...
	if ( ! function_exists( 'wbs_locate_template' ) ) {
		...
		function wbs_locate_template( $template_name, $template_path = '', $default_path = '' ) {
			if ( ! $template_path ) {
				$template_path = '/woo-boost-sales/';
			}
			if ( ! $default_path ) {
				$default_path = VI_WOO_BOOSTSALES_TEMPLATES;
			}
			...
			return apply_filters( 'wbs_locate_template', $template, $template_name, $template_path );
		}
	}
}
if ( ! function_exists( 'wbs_wc_dropdown_variation_attribute_options' ) ) {
	function wbs_wc_dropdown_variation_attribute_options( $args = array() ) {
		...
		if ( false === $args['selected'] && $args['attribute'] && $args['product'] instanceof WC_Product ) {
			...
			$args['selected'] = isset( $_REQUEST[ $selected_key ] ) ? wc_clean( wp_unslash( $_REQUEST[ $selected_key ] ) ) : $args['product']->get_variation_default_attribute( $args['attribute'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.NonceVerification.Recommended
		}
		...
		echo apply_filters( 'wbs_woocommerce_dropdown_variation_attribute_options_html', $html, $args ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
if ( ! function_exists( 'wbs_woocommerce_quantity_input' ) ) {
	...
	function wbs_woocommerce_quantity_input( $args = array(), $product = null, $echo = true ) {
		...
		ob_start();
		wbs_get_template( 'single-product/add-to-cart/quantity-input.php', $args, '', VI_WOO_BOOSTSALES_TEMPLATES );
