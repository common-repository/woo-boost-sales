...
//			add_action( 'wp_ajax_wbs_set_notice', array( $this, 'wbs_set_notice' ) );
	...
	public function wbs_set_notice() {
		$product_id = isset( $_GET['product_id'] ) ? wc_clean( $_GET['product_id'] ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$notices    = '';
	...
	public function init_scripts_side_cart() {
		if ( $this->auto_open_cart !== false && isset( $_POST['add-to-cart'], $_POST['quantity'] ) && sanitize_text_field( $_POST['add-to-cart'] ) && sanitize_text_field( $_POST['quantity'] ) ) {// phpcs:ignore WordPress.Security.NonceVerification.Missing
			...
			wp_enqueue_script( 'woo-boost-sales-woo-side-cart-script', VI_WOO_BOOSTSALES_JS . 'woo-boost-sales-side-cart' . $js_ext, array(
				'jquery',
				'xoo-wsc'
			), VI_WOO_BOOSTSALES_VERSION, false );
		}
	}
	...
	public function init_scripts() {
		...
		wp_localize_script( 'woo-boost-sales', 'woocommerce_boost_sales_params', array(
				...
				/* translators: %s: auto redirect time */
