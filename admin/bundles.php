...
if ( ! defined( 'VI_WOO_BOOSTSALES_TEMPLATES' ) ) {
	define( 'VI_WOO_BOOSTSALES_TEMPLATES', VI_WOO_BOOSTSALES_DIR . "templates" . DIRECTORY_SEPARATOR );
}
if ( ! class_exists( 'VI_WOO_BOOSTSALES_Admin_Bundles' ) ) {
	/**
	 * Admin class.
	 * The class manage all the admin behaviors.
	 */
	class VI_WOO_BOOSTSALES_Admin_Bundles {
		...
		public function __construct() {
			...
			add_action( 'init', array( $this, 'register_wbs_bundle_product_type' ), 1 );
		...
		public function init() {
			require_once VI_WOO_BOOSTSALES_INCLUDES . "wbs_bundled_item.php";
		}

		public function register_wbs_bundle_product_type() {
			require_once VI_WOO_BOOSTSALES_INCLUDES . "wbs_bundled_item.php";
			require_once VI_WOO_BOOSTSALES_INCLUDES . "wbs_product_bundle.php";
		}
		...
		public function woocommerce_admin_order_item_count( $count, $order ) {
			...
			if ( $counter > 0 ) {
				...
				/* translators: %1$s: order item count, %2$s: bundle element count */
				return sprintf( _n( '%1$s item [%2$s bundled elements]', '%1$s items [%2$s bundled elements]', $non_bundled_count, 'woo-boost-sales' ), $non_bundled_count, $counter );
			}
		...
		public function product_type_selector( $types ) {
			$types['wbs_bundle'] = _x( 'WBS Product Bundle', 'Admin: type of product', 'woo-boost-sales' );
		...
		public function wbs_wcpb_admin_product_bundle_data( $metabox_id, $product_id, $item_data, $post_id ) {
            ...
            <table>
                <tr>
                    <td><?php esc_html_x( 'Quantity', 'Admin: quantity of the bundled product.', 'woo-boost-sales' ) ?></td>
                    <td>
                        <input type="number" size="4" value="<?php echo esc_attr( $bp_quantity ) ?>"
                               name="_wbs_wcpb_bundle_data[<?php echo esc_attr( $metabox_id ); ?>][bp_quantity]"
		...
		public function add_product_in_bundle() {
			check_ajax_referer('_wbs_product_nonce','nonce');
			...
			ob_start();
			include VI_WOO_BOOSTSALES_TEMPLATES . 'admin/admin-bundled-product-item.php';
			echo VI_WOO_BOOSTSALES_Data::wp_kses_post( ob_get_clean() );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		public function woocommerce_product_data_tabs( $product_data_tabs ) {
			$product_data_tabs['wbs_bundled_options'] = array(
				'label'  => __( 'Bundle Options', 'woo-boost-sales' ),
		...
		public function woocommerce_product_data_panels() {
            ...
            <div id="wbs_bundled_product_data" class="panel woocommerce_options_panel wc-metaboxes-wrapper">
                <?php wp_nonce_field( '_wbs_product_nonce', '_wbs_product_nonce' ) ?>
                <div class="options_group wbs-wcpb-bundle-metaboxes-wrapper">

                    <div id="wbs-wcpb-bundle-metaboxes-wrapper-inner">
                        <p class="form-field">
                            <label for="_wbs_dynamic_price"><?php esc_html_e( 'Use dynamic price', 'woo-boost-sales' ); ?></label>
                            <input id="_wbs_dynamic_price" type="checkbox"
                                   name="_wbs_dynamic_price"
                                   value="1" disabled><?php esc_html_e( 'Premium feature', 'woo-boost-sales' ); ?>
                        </p>
                        <p class="form-field wbs-price-info">
                            <label for="wbs_bundle_price"><?php /* translators: %s: currency symbol */
                                printf( esc_html__( 'Price(%s)', 'woo-boost-sales' ), esc_html( get_woocommerce_currency_symbol() ) ); ?></label>
                            <input id="wbs_bundle_price" type="text" class="short wc_input_price"
                                   name="wbs_bundle_price"
                                   value="<?php echo esc_attr( $price ) ?>">
                        </p>
                        <p class="toolbar">
                            <a href="#" class="close_all"><?php esc_html_e( 'Close all', 'woo-boost-sales' ); ?></a>
                            <a href="#" class="expand_all"><?php esc_html_e( 'Expand all', 'woo-boost-sales' ); ?></a>
                        </p>
                            ...
                            <span class="wbs-wcpb-bundled-prod-toolbar-wrapper">
                                <span class="wbs-wcpb-bundled-prod-selector">
                                    <select type="hidden" class="wc-product-search" style="width: 250px;"
                                            ...
                                            data-placeholder="<?php esc_html_e( 'Search for a product&hellip;', 'woo-boost-sales' ); ?>"
                                            data-exclude="<?php echo esc_attr( $post->ID ); ?>">
									</select>
                                </span>
                                <button type="button" id="wbs-wcpb-add-bundled-product"
                                        class="button button-primary"><?php esc_html_e( 'Add Product', 'woo-boost-sales' ); ?></button>
                            </span>
                        </p>
                    </div>
                </div>
		...
		public function woocommerce_process_product_meta( $post_id ) {
			if ( isset( $_REQUEST['_wbs_product_nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_REQUEST['_wbs_product_nonce'] ), '_wbs_product_nonce' ) ) {
				return;
			}
			$bundle_data = isset( $_POST['_wbs_wcpb_bundle_data'] ) ? wc_clean( $_POST['_wbs_wcpb_bundle_data'] ) : false;
		...
		public function save_price( $post_id ) {
			...
			$bundle_price = isset( $_POST['wbs_bundle_price'] ) ? wc_clean( wp_unslash( $_POST['wbs_bundle_price'] ) ) : '';
		...
		public function admin_enqueue_scripts() {
			...
			if ( 'product' == $screen->id ) {
				wp_enqueue_style( 'wbs-wcpb-admin-styles', VI_WOO_BOOSTSALES_CSS . 'bundle-admin.css', [], VI_WOO_BOOSTSALES_VERSION );
				wp_enqueue_script( 'jquery-ui-sortable' );
				wp_enqueue_script( 'woo-boost-sales', VI_WOO_BOOSTSALES_JS . 'bundles.js', array( 'jquery' ), VI_WOO_BOOSTSALES_VERSION, false );
				wp_localize_script( 'woo-boost-sales', 'wbs_product_params', array( 'nonce' => wp_create_nonce( '_wbs_product_nonce' ) ) );
			}
		}
...
/**
 * Unique access to instance of VI_WOO_BOOSTSALES_Admin_Bundles class
 *
 * @return \VI_WOO_BOOSTSALES_Admin_Bundles
 * @since 1.0.0
 */
function VI_WOO_BOOSTSALES_Admin_Bundles() {
	return VI_WOO_BOOSTSALES_Admin_Bundles::get_instance();
}
