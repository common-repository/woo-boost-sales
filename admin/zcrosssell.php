...
class VI_WOO_BOOSTSALES_Admin_ZCrosssell {
	...
	public function get_crs_select( $p_id ) {
		...
		if ( $prds->has_child() && $prds->get_type() == 'variable' ) {
			...
			if ( count( $children ) ) {
				foreach ( $children as $child ) {
					$sql_parent    = $wpdb->prepare( "SELECT post_id FROM " . $wpdb->prefix . "postmeta WHERE meta_key = '_wbs_wcpb_bundle_data' AND meta_value LIKE '%s'", '%' . $child . '%' );// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.QuotedSimplePlaceholder
					$result_parent = $wpdb->get_results( $sql_parent, OBJECT );// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		...
		$sql    = $wpdb->prepare( "SELECT post_id FROM " . $wpdb->prefix . "postmeta WHERE meta_key = '_wbs_wcpb_bundle_data' AND meta_value LIKE '%s'", '%' . $p_id . '%' );// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.QuotedSimplePlaceholder
		$result = $wpdb->get_results( $sql, OBJECT );// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		if ( ! $result ) {
			return 0;
		} else {
	...
	public function wbs_c_save_product() {
		check_ajax_referer('_wbs_cross_sells_search', 'nonce');
		global $wp_error;
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
	...
	public function enqueue_scripts() {
		...
		if ( $page == 'woo-boost-sales-crosssell' ) {
			...
			wp_enqueue_script( 'select2-v4', VI_WOO_BOOSTSALES_JS . 'select2.js', array( 'jquery' ), '4.0.3', false );
			...
			wp_enqueue_script( 'woo-boost-sales-crosssell-admin', VI_WOO_BOOSTSALES_JS . 'woo-boost-sales-crosssell-admin.js', array( 'jquery' ), VI_WOO_BOOSTSALES_VERSION, false );
			wp_localize_script( 'woo-boost-sales-crosssell-admin', 'wbs_crosssell_admin_params', array(
				'url' => admin_url( 'admin-ajax.php' ),
				'nonce' => wp_create_nonce( '_wbs_cross_sells_search' )
			) );
		}
	}
	...
	public function page_callback() {
		if ( isset( $_REQUEST['_wsm_nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_REQUEST['_wsm_nonce'] ), '_wbs_cross_sells_search' ) ) {
			return;
		}
        ...
        <div class="wrap">
			...
			if ( ! $this->settings->get_option( 'enable' ) || ! $this->settings->get_option( 'crosssell_enable' ) ) {
				?>
                <div class="error">
                    <p><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( __( 'Cross-sells feature is currently disabled. <span class="wbs-crosssells-ajax-enable button button-primary">Enable now</span>', 'woo-boost-sales' ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
                </div>
				<?php
			}
			...
			if ( $the_query->have_posts() ) {
				...
				echo VI_WOO_BOOSTSALES_Data::wp_kses_post( $pagination_html );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				?>
						...
						while ( $the_query->have_posts() ) {
                            ...
                            <tr id="product-<?php echo esc_attr( $p_id ) ?>">
                                <td class="product column-product has-row-actions column-primary"
                                    ...
                                    <a href="<?php echo esc_url( 'post.php?action=edit&post=' . $p_id ) ?>"><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( '[#' . get_the_ID() . '] ' . the_title( '', '', '' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a>
                                </td>
                                <td data-id="<?php echo esc_attr( $p_id ) ?>" class="name column-cross-sells"
                                    ...
                                    <div class="product-search-crs-container">
                                        <select multiple="multiple" name="_wbs_cross_sell"
                                                class="product-search-crs u-product-<?php echo esc_attr( get_the_ID() ) ?>">
											<?php
											if ( $bundle_id ) {
