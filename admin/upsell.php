...
class VI_WOO_BOOSTSALES_Admin_Upsell {
	...
	public function wbs_u_save_product() {
	    check_ajax_referer('_wbs_upsells_search', 'nonce' );
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
	...
	public function wbs_search_product() {
		...
		$arg            = array(
			...
			's'              => $keyword,
			'fields'         => 'ids',
			'post__not_in'   => array( $p_id ),// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in
//			'tax_query'      => array(
//				'relation' => 'AND',
//				array(
...
//			)
	...
	public function enqueue_scripts() {
		...
		if ( $page == 'woo-boost-sales-upsell' ) {
			...
			wp_localize_script( 'woo-boost-sales-upsell-admin', 'wbs_upsell_admin_params', array(
				...
				'nonce' => wp_create_nonce( '_wbs_upsells_search' )
			) );
		}
	}
	...
	public function page_callback() {
		if ( isset( $_REQUEST['_wsm_nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_REQUEST['_wsm_nonce'] ), '_wbs_upsells_search' ) ) {
			return;
		}
        ...
        <div class="wrap">
			...
			if ( ! $this->settings->get_option( 'enable' ) || ! $this->settings->get_option( 'enable_upsell' ) ) {
				?>
                <div class="error">
                    <p><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( __( 'Up-sells feature is currently disabled. <span class="wbs-upsells-ajax-enable button button-primary">Enable now</span>', 'woo-boost-sales' ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
                </div>
				<?php
			}
			if ( $the_query->have_posts() ) {
                ...
                <div class="list-products">
                    <table class="wp-list-table widefat fixed striped">
						...
						while ( $the_query->have_posts() ) {
							$the_query->the_post();
							?>
                            <tr id="product-<?php echo esc_attr( get_the_ID() ) ?>">
                                <td class="product column-product has-row-actions column-primary"
                                    data-colname="product-name">
                                    <a href="<?php echo esc_url( 'post.php?action=edit&post=' . get_the_ID() ) ?>"><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( '[#' . get_the_ID() . '] ' . the_title( '', '', '' ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a>
                                </td>
                                <td data-id="<?php echo esc_attr( get_the_ID() ) ?>" class="name column-up-sells"
                                    ...
                                    <select multiple="multiple" name="_wbs_up_sell"
                                            class="product-search u-product-<?php echo esc_attr( get_the_ID() ) ?>">
											...
											foreach ( $products as $product ) {
												...
												if ( $data ) {
                                                    ...
                                                    <option selected="selected"
                                                            value="<?php echo esc_attr( $data->get_id() ) ?>"><?php echo VI_WOO_BOOSTSALES_Data::wp_kses_post( ' (#' . $product . ') ' . $data->get_title() . $parent . $out_stock );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></option>
													<?php
												}
											}
										}
                                ...
                                <td class="email column-action product-action-<?php echo esc_attr( get_the_ID() ) ?>"
